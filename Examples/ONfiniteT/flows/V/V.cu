  ...
  namespace Flows
  {
    ...
    V_integrator::V_integrator(const V_integrator& other)
        : quadrature_provider(other.quadrature_provider), grid_sizes(other.grid_sizes), jac_grid_sizes(other.jac_grid_sizes), x_extent(other.x_extent), 
          ...
          integrator_AD(std::make_unique<DiFfRG::IntegratorTBB<3, autodiff::real, V_kernel<__REGULATOR__>>>(other.quadrature_provider, other.jac_grid_sizes, other.x_extent, other.json))
    {
    }
    ...
    std::future<autodiff::real> V_integrator::request_AD(const double k, const double N, const double T, const double rhoPhi, const autodiff::real m2Pi, const autodiff::real m2Sigma)
    {
